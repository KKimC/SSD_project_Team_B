...
def _read_compare(
    receiver: SSDController, lba: int, write_value: str
):
...
class FullWriteAndReadCommand(ScriptCommandType):
    LIST_SCRIPT_TYPE = ["1_", TestScriptType.FULL_WRITE_AND_READ]
...
class PartialLbaWriteCommand(ScriptCommandType):
    LIST_SCRIPT_TYPE = ["2_", TestScriptType.PARTIAL_LBA_WRITE]
    ...
    def execute(self):
        for _ in range(30):
            ...
            for read_lba_address in range(5):
                if _read_compare(self.receiver, read_lba_address, write_value) == "FAIL":
                    raise ExitException
        logger.print(
            f"{self.__class__.__name__}.{inspect.currentframe().f_code.co_name}()",
            f"SCRIPT_2",
...
class WriteReadAgingCommand(ScriptCommandType):
    LIST_SCRIPT_TYPE = ["3_", TestScriptType.WRITE_READ_AGING]
    ...
    def execute(self):
        ...
        for _ in range(200):
            ...
            for i, lba_address in enumerate(lba_address_list):
                if _read_compare(self.receiver, lba_address, write_value_list[i]) == "FAIL":
                    raise ExitException
        logger.print(
            f"{self.__class__.__name__}.{inspect.currentframe().f_code.co_name}()",
            f"SCRIPT_3",
...
class EraseAndAgingCommand(ScriptCommandType):
    LIST_SCRIPT_TYPE = ["4_", TestScriptType.ERASE_AND_AGING]
...
def get_matched_script(command_str: str):
    for script_command in list_script_commands:
        for name in script_command.LIST_SCRIPT_TYPE:
            if name == command_str:
